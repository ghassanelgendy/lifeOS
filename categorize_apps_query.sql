-- Quick query to categorize all existing app rows
...
SET category = CASE
  ...
  WHEN LOWER(TRIM(app_name)) = 'shellexperiencehost' THEN 'WebBrowsing'
  WHEN LOWER(TRIM(app_name)) = 'msiexec' THEN 'WebBrowsing'
  WHEN LOWER(TRIM(app_name)) = 'startmenuexperiencehost' THEN 'WebBrowsing'
  WHEN LOWER(TRIM(app_name)) = 'explorer' THEN 'WebBrowsing'
  WHEN LOWER(TRIM(app_name)) = 'msedge' THEN 'WebBrowsing'
  ...
  -- Pattern matching fallbacks (regex)
  ...
  -- Keep existing category if it's already set and not 'Uncategorized'
  ELSE COALESCE(NULLIF(category, 'Uncategorized'), 'Uncategorized')
END,
