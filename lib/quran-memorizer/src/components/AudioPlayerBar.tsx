      ...
      <div
        ...
        className={`fixed z-[10000] font-arabic-title text-right
          ...
          ${isBarHidden || forceSmall
            ? 'scale-[0.78] translate-y-[10px] opacity-55 bg-card/80 backdrop-blur-md'
            : 'scale-100 translate-y-0 opacity-100 bg-card/95 backdrop-blur-2xl'}
          /* Desktop / MD+: Perfectly docked and aligned with the sidebar line */
          md:bottom-0 md:translate-x-0 md:translate-y-0 md:scale-100 md:opacity-100 md:right-0 md:w-auto md:max-w-none md:h-14 md:rounded-none md:border-t md:border-x-0 md:border-b-0 md:border-border md:bg-card/95 md:backdrop-blur-xl md:px-6 md:py-2
        ...
        <div className="w-full max-w-7xl mx-auto flex items-center justify-between gap-2 md:gap-4 text-foreground h-full">
          ...
          <div className="flex items-center gap-2 min-w-0 shrink">
            ...
            <div className="hidden md:flex items-center gap-2">
              <Volume2 className="size-4 text-emerald-600 dark:text-emerald-400 shrink-0" />
              <select
                value={reciter.id}
                onChange={(e) => {
                ...
                className="bg-secondary text-foreground text-xs font-bold rounded-xl px-2.5 py-1.5 border border-border focus:outline-none focus:ring-1 focus:ring-emerald-500 cursor-pointer"
              >
                {RECITERS.map((r) => (
            ...
            <div
              ...
              className="flex items-center gap-1.5 text-[10px] bg-secondary hover:bg-secondary/80 border border-border px-2 py-1 rounded-full cursor-pointer active:scale-95 transition-all truncate"
            >
              <span className="md:hidden text-emerald-600 dark:text-emerald-400 font-bold truncate max-w-[70px]">
              ...
              {repeatSettings.verseRepeats > 1 && (
                <span className="text-[9px] text-emerald-600 dark:text-emerald-400 font-mono font-bold bg-emerald-500/15 px-1 py-0.2 rounded-md shrink-0">
                  {currentVerseRepeat}/{repeatSettings.verseRepeats}
                </span>
              )}
              {isPlaying && isDelaying && (
                <span className="animate-pulse text-amber-600 dark:text-amber-400 text-[9px] font-bold shrink-0">
                  سكوت...
                </span>
              )}
            </div>
          ...
          <div dir="rtl" className="flex items-center gap-0.5 md:gap-2 shrink-0">
            <button
              onClick={onPrev}
              className="p-1.5 rounded-full hover:bg-secondary text-foreground active:scale-90 transition-all cursor-pointer"
              title="الآية السابقة"
            >
            ...
            <button
              onClick={onStop}
              className="p-1 rounded-full hover:bg-secondary text-muted-foreground hover:text-foreground active:scale-90 transition-all cursor-pointer"
              title="إيقاف"
            >
          ...
          <div className="hidden md:flex items-center gap-2 text-xs shrink-0">
            {/* Verse Repeats */}
            <div className="flex items-center gap-1.5 bg-secondary px-2.5 py-1 rounded-xl border border-border">
              <Repeat className="size-3.5 text-emerald-600 dark:text-emerald-400 shrink-0" />
            ...
            <div className="flex items-center gap-1.5 bg-secondary px-2.5 py-1 rounded-xl border border-border">
              ...
              >
                {[0, 1, 2, 3, 5, 8].map((sec) => (
                  <option key={sec} value={sec} className="bg-card text-foreground">
                    {sec === 0 ? 'بدون' : `${sec}ث`}
                  </option>
                ))}
              </select>
            ...
            <select
              ...
              className="bg-secondary text-foreground text-[11px] font-bold rounded-xl px-2 py-1 border border-border focus:outline-none cursor-pointer"
            >
              {[0.75, 1.0, 1.25, 1.5].map((speed) => (
                <option key={speed} value={speed} className="bg-card text-foreground">
                  {speed}x
                </option>
              ))}
            </select>
          ...
          <div className="md:hidden flex items-center">
            <button
              onClick={() => setShowSettingsDrawer(true)}
              className="flex items-center gap-1 px-2 py-1 rounded-full bg-secondary hover:bg-secondary/80 text-foreground text-[10px] font-bold border border-border active:scale-95 transition-all cursor-pointer"
              title="إعدادات الصوت والتكرار"
            >
              <SlidersHorizontal className="size-3 text-emerald-600 dark:text-emerald-400" />
          ...
          >
            ...
            <div className="space-y-1.5">
              ...
              <select
                ...
                className="w-full bg-secondary/80 text-foreground text-xs font-bold rounded-xl px-3 py-2.5 border border-border focus:outline-none focus:ring-1 focus:ring-emerald-500"
              >
                {RECITERS.map((r) => (
                  <option key={r.id} value={r.id} className="bg-card text-foreground">
                    {r.name}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-muted-foreground flex items-center gap-1.5">
                  <Repeat className="size-3.5 text-emerald-600 dark:text-emerald-400" />
                ...
                <select
                  ...
                  className="w-full bg-secondary/80 text-foreground text-xs font-bold rounded-xl px-3 py-2 border border-border focus:outline-none"
                >
                  {[1, 2, 3, 5, 7, 10, 20].map((num) => (
                    <option key={num} value={num} className="bg-card text-foreground">
                      {num} مرات
