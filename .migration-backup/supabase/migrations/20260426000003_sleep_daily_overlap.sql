...
drop view if exists public.analytics_daily_sleep;

create view public.analytics_daily_sleep as
with split_segments as (
  select
    st.user_id,
    day_bucket.day_start::date as date,
    st.stage,
    greatest(st.started_at, day_bucket.day_start::timestamp with time zone) as overlap_started_at,
    least(st.ended_at, (day_bucket.day_start + interval '1 day')::timestamp with time zone) as overlap_ended_at
  from public.sleep_stages st
  cross join lateral generate_series(
    st.started_at::date::timestamp,
    (st.ended_at - interval '1 second')::date::timestamp,
    interval '1 day'
...
select
  user_id,
  date,
  round(sum(case when coalesce(stage, '') <> 'Awake' then minutes else 0 end))::int as total_minutes,
...
comment on view public.analytics_daily_sleep is
  'Daily sleep stage totals split by actual per-day overlap; total_minutes excludes awake time.';
