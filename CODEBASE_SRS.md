...
- **iOS Native (`Notes.ios.tsx`)**: Apple Notes aesthetic, grouped inset card lists, iOS header navigation, touch gestures, haptics (`triggerHaptics`), 3D long-press context menus, elevated FAB new note button, and AI action toolbar positioned safely above the LiquidTabBar.
...
The reader shall provide an iOS-native liquid glass header with compact Surah and Juz dropdowns, dynamic Fehres (Index) pill button reflecting the active Surah on the current Medina Mushaf page across all view modes, and an iOS bottom sheet for secondary controls. The Fehres sheet shall be keyboard-aware with visual viewport tracking and non-intrusive focus management on iOS. Left-to-right swipe gestures on the Quran reader shall be strictly isolated to page turning, suppressing global sidebar drawer triggers. Navigating or deep-linking to an Ayah shall auto-scroll and highlight the verse within the view.
...
Long-pressing any verse in the reader shall trigger an iOS 3D lift visual effect and open a frosted glass context menu providing 5 distinct actions: Hide Ayah (temporary session test), Set Memorization Checkpoint, Set Reading Checkpoint, Bookmark to Notes, and Explain (Tafsir Al-Muyassar sheet drawer). Setting a checkpoint shall immediately update `quran_khatmah_plans` and localStorage with the exact Ayah and page, update the linked habit's description with the latest Ayah position, and mark today's habit log completed. Conversely, completing a Quran habit advances the Wird and updates the checkpoint accordingly. The menu shall support native iOS hold-and-slide-to-select touch gestures with global touchmove/touchend tracking, haptic tick feedback, and release-to-choose execution. Selected ayahs shall use a non-intrusive underline without layout shift or wiggles, and Tafsir Al-Muyassar shall slide smoothly from the bottom.
