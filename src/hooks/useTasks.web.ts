...
export async function syncTaskCompletionToLinkedNote(
  ...
  try {
    ...
    if (!note) {
      // Find note containing this task title in its body or analysis
    ...
    if (isCompleted) {
      // Replace "- [ ] <task>" or "- <task>" with "- [x] <task>"
...
export function useToggleTask() {
  ...
  return useMutation({
    ...
    onMutate: async (id: string) => {
      ...
      const currentTask = (queryClient.getQueryData(TASKS_KEY) as Task[] | undefined)?.find(
        (t) => t.id === id
      );
      if (currentTask) pendingToggleOriginalCompleted.set(id, currentTask.is_completed);
