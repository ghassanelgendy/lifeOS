...
export default function NotesWeb() {
  ...
  const handleBulkDeleteSelected = async () => {
    ...
    try {
      ...
      if (selectedNoteIds.has(activeId)) startNewNote();
    ...
    <div className="flex flex-col h-full min-h-0 space-y-4 pt-3 md:pt-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 flex-1 min-h-0">
          ...
          <div className="p-3 border-b border-border space-y-2 shrink-0">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search all notes (title & content)..."
        ...
        <section className="lg:col-span-4 rounded-xl border border-border bg-card overflow-hidden flex flex-col min-h-0">
          <div className="p-3 border-b border-border flex items-center justify-between text-xs text-muted-foreground shrink-0 gap-2">
            ...
            {activeFolderFilter !== BRAIN_DUMP_NOTES && filteredNotes.length > 0 && (
              isSelectMode ? (
                <div className="flex items-center gap-2 shrink-0">
                  <span className="tabular-nums">{selectedNoteIds.size} selected</span>
                  <button type="button" onClick={handleSelectAllInView} className="text-primary hover:underline">
                    Select all
                  </button>
                  <button
                    type="button"
                    ...
                    className="text-destructive hover:underline disabled:opacity-40 disabled:hover:no-underline"
                  >
                    Delete
                  </button>
                  <button type="button" onClick={handleExitSelectMode} className="hover:text-foreground">
                    Cancel
                  </button>
                </div>
              ) : (
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button type="button" onClick={() => setIsSelectMode(true)} className="text-primary hover:underline">
                    Select
                  </button>
                </div>
              )
              ...
              filteredNotes.map((note) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "w-full text-left p-3.5 rounded-xl border transition-all relative",
                      isChecked
                        ? "border-primary/40 bg-primary/10 ring-1 ring-primary/30"
                        : isActive
                        ? "border-border bg-secondary shadow-sm"
                        : "border-border/60 bg-card hover:border-border hover:bg-secondary/60 shadow-sm hover:shadow-md"
                    )}
                  >
      ...
      <ConfirmSheet
        ...
        title="Delete Notes"
        message={`Are you sure you want to delete ${selectedNoteIds.size} note${selectedNoteIds.size === 1 ? '' : 's'}? This can't be undone.`}
        confirmLabel="Delete"
