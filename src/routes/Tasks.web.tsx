...
import { Plus, Calendar as CalendarIcon, Check, Edit2, ChevronRight, ChevronDown, Star, CalendarDays, CheckCircle2, Flag, Tag as TagIcon, Repeat, ListTodo, Trash2, Clock, Sun, ArrowRight, CircleSlash2, ArrowUpDown, Mic, Users, Copy, Wand2, Sparkles, Loader2, Columns3, Search, X, CheckSquare, ListFilter, FolderInput } from 'lucide-react';
...
import type { Task, Tag, TaskList, CreateInput, TaskPriority, TaskRecurrence, TaskRecurrenceEndType, CalendarEvent } from '../types/schema';
...
export default function Tasks() {
  ...
  const [batchMoveListId, setBatchMoveListId] = useState<string | null>(null);
  ...
  const handleQuickAddTitleChange = (rawTitle: string) => {
    ...
    // Keep picker trigger active so users can choose via inline shortcuts (~, !, #).
  ...
  const handleSelectAllVisible = () => {
    ...
    mainTasksToRender.forEach((t) => {
      if (t?.id && !isHabitTaskId(t.id)) ids.add(t.id);
    });
    if (showCompleted) {
  ...
  const handleSelectBraindumpTasks = () => {
    ...
    allTasks.forEach((t) => {
      if (!t?.id || isHabitTaskId(t.id)) return;
  ...
  const unscheduledTasks = useMemo(() => {
    return allTasks.filter(
      (t) => Boolean(t && typeof t.id === 'string' && !t.is_completed && !t.is_wont_do && !t.due_date && !isHabitTaskId(t.id))
    );
  }, [allTasks]);

  // Calculate awake window from user's sleep metrics (shared by both schedule modes)
  const computeAwakeWindow = useCallback(() => {
    let wakeHour = 8;
    let bedHour = 23.5;
  ...
  }, [avgBedtimeMinutes, avgSleepMinutes]);
  ...
  const currentWeekScheduledTasks = useMemo(() => {
    ...
    return combined.filter((t) => {
      ...
      seen.add(t.id);
      return !t.is_completed && !t.is_wont_do && !!t.due_date && !isHabitTaskId(t.id);
    });
  ...
  const selectedTasks = useMemo(() => {
    if (selectedTaskIds.size === 0) return [];
    return allTasks.filter((t) => Boolean(t && typeof t.id === 'string' && selectedTaskIds.has(t.id) && !isHabitTaskId(t.id) && !t.is_wont_do));
      ...
      <main ref={mainContentRef} className="flex-1 min-w-0 flex flex-col overflow-hidden pb-[env(safe-area-inset-bottom)]">
        ...
        <header className="flex items-center justify-between p-4 border-b border-border">
          ...
          <div className="flex items-center gap-2">
            {isSearchOpen ? (
              <div className="flex items-center gap-1.5 rounded-full border border-border bg-card/70 pl-3 pr-1.5 h-9 w-[160px] sm:w-[220px] transition-all">
            ...
            <button
              ...
              className={cn(
                "h-9 px-2.5 rounded-full border transition-all flex items-center gap-1.5 text-xs font-medium shrink-0",
                isSelectionMode
...
Return ONLY raw JSON.`;
          ...
          {displayMode === 'board' ? (
            ...
            <TaskGanttChart tasks={searchedTasks} taskLists={taskLists} onEdit={handleEditTask} />
          ) : (
          <>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-1 bg-secondary/60 p-0.5 rounded-lg text-[11px]">
            ...
            <button
              type="button"
              onClick={() => {
                setSmartScheduleMode('unscheduled');
                setSmartSchedulePlan(generatePlanForMode('unscheduled', smartScheduleHorizon));
              }}
              className={cn(
                "flex-1 px-2.5 py-1.5 rounded-md transition-colors",
                smartScheduleMode === 'unscheduled' ? "bg-card font-semibold shadow-xs text-foreground" : "text-muted-foreground hover:text-foreground"
              )}
            >
            ...
            <button
              type="button"
              onClick={() => {
                setSmartScheduleMode('reorganize');
                setSmartSchedulePlan(generatePlanForMode('reorganize', smartScheduleHorizon));
              }}
              className={cn(
                "flex-1 px-2.5 py-1.5 rounded-md transition-colors",
                smartScheduleMode === 'reorganize' ? "bg-card font-semibold shadow-xs text-foreground" : "text-muted-foreground hover:text-foreground"
              )}
            >
              ...
              <div className="flex items-center gap-1 bg-secondary/60 p-0.5 rounded-lg text-[11px]">
                <button
                  ...
                  onClick={() => {
                    ...
                    setSmartSchedulePlan(generatePlanForMode(smartScheduleMode, 'week'));
                  }}
                  className={cn(
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    setSmartSchedulePlan(generatePlanForMode(smartScheduleMode, 'month'));
                  }}
                  className={cn(
                    "px-2 py-0.5 rounded-md transition-colors",
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1 max-h-60 overflow-y-auto">
            <button
              ...
              className="w-full text-left px-3 py-2 rounded-lg hover:bg-secondary flex items-center gap-2 text-sm"
            >
          ...
          <div className="flex flex-wrap gap-2 max-h-60 overflow-y-auto">
            {tags.map((tag) => (
              <button
                ...
                className="px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 border border-border hover:border-foreground/40 transition-colors"
...
function TaskItem({ task, tags, onToggle, onEdit, onDelete, onWontDo, formatDueDate, onToggleSubtask, isSelectionMode, isSelected, onSelect }: TaskItemProps) {
  ...
  return (
    ...
    >
      <div
        ...
        onClick={() => {
          if (isSelectionMode && onSelect && !isHabit) {
            onSelect(task.id);
            return;
          }
          if (!isHabit) {
      ...
      >
        {isSelectionMode && !isHabit ? (
          <button
            ...
            className={cn(
              "relative mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-lg border-2 transition-all duration-200",
              isSelected
