    ...
    <FluentProvider theme={theme} className="w-full">
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 font-sans text-foreground">
        ...
        <div className="flex items-center justify-between">
          ...
          {activeTab === 'transactions' && (
            <FluentButton icon={<Plus size={18} />} onClick={() => handleOpenModal()} size="medium" appearance="primary" aria-label="Add transaction">
              Add
            </FluentButton>
          )}
        ...
        <TabList
          ...
          onTabSelect={(_e, data) => setActiveTab(data.value as FinanceTab)}
          appearance="subtle"
          size="medium"
          className="w-full border-b border-border mb-6"
        >
          <Tab value="transactions" icon={<ArrowLeftRight size={16} />}>
            Transactions
          </Tab>
          <Tab value="banks" icon={<Wallet size={16} />}>
            Banks
          </Tab>
          <Tab value="investments" icon={<Briefcase size={16} />}>
            Investments
          </Tab>
        </TabList>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-xl border border-border bg-card p-4 md:p-6 shadow-sm overflow-hidden">
          <TabList
            ...
            onTabSelect={(_e, data) => setGraphType(data.value as 'category' | 'overtime' | 'accounts')}
            appearance="subtle"
            size="small"
            className="w-full border-b border-border mb-4"
          >
            <Tab value="category" icon={<PieChart size={14} />}>
              By category
            </Tab>
            <Tab value="overtime" icon={<TrendingUpIcon size={14} />}>
              Over time
            </Tab>
            {selectedBank === '' && (
              <Tab value="accounts" icon={<Landmark size={14} />}>
                By account
              </Tab>
            )}
          </TabList>
        ...
        {viewAllTransactions && (
          <div className="px-4 pb-3 border-b border-border flex flex-wrap items-center gap-2">
            ...
            <TabList
              ...
              onTabSelect={(_e, data) => setDateFilterMode(data.value as 'all' | 'month' | 'day')}
              appearance="subtle"
              size="small"
              className="border-b border-border mr-2"
            >
      ...
      >
        <div className="space-y-4">
          ...
          <div className="rounded-xl bg-secondary/30 p-3 space-y-1 border border-border">
