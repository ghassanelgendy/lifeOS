...
- **Reorganize This Week Mode:** The same Schedule modal offers a second mode — "Reorganize This Week" — that re-spreads tasks *already* scheduled for the current calendar week across a wider or narrower horizon ("Stretch Over Week" vs "Stretch Over Month") using the same conflict-free awake-slot engine, treating every other active task and calendar event as a fixed obstacle. This lets a week that got front-loaded with due dates be evenly stretched out instead of requiring each task to be manually re-dated.
