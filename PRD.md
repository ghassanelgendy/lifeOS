...
- **Search:** A search toggle in the Tasks page header (PC web, iOS, and desktop/Pake) expands into an inline text input that filters the currently active view (smart list, custom list, or tag) by task title, description, and tag names, case-insensitively; clearing or closing the search (X button or Escape) restores the unfiltered view. Search applies across List, Board, and Timeline display modes on PC web.
...
- **Reorganize Modes (This Week & Selected Tasks):** The Schedule modal offers reorganization modes — "Reorganize This Week" and "Reorganize Selected" — that re-spread existing tasks across a wider or narrower horizon ("Stretch Over Week" vs "Stretch Over Month") using the conflict-free awake-slot engine, treating all other active tasks and calendar events as fixed obstacles. Users can also select arbitrary tasks in batch mode and trigger "Reorganize" directly from the selection action bar or header.
...
- **Date-Aware Scheduling:** The Smart Unscheduled Tasks Scheduler and Reorganize modes detect an explicit date named in a task's title (a weekday, "tomorrow", a written date, or a numeric `D/M` date — interpreted as DAY/MONTH, e.g. "10/9" = 10 September) and pin that task to the named date instead of dropping it into whichever slot the free-slot algorithm finds next; the numeric-date parser (`parseTaskInput`) also runs at Quick Add time so typed dates like "10/9" are captured as `due_date` immediately rather than never being recognized.
...
- **Dynamic Investment Platforms:** Investment platforms (accounts) are fully user-managed — add, rename, or remove them from a "Platforms" panel on the Investments tab. Thndr and Fawry are seeded once as starting suggestions only, not a fixed/hardcoded set; removing a platform also deletes its transactions (confirmed with a count before removal), while renaming keeps all transactions linked (they reference the platform by id, not by name).
- **Investment Transaction Types:** Each investment transaction is tagged Deposit, Withdrawal, Profit, or Loss (shown as a badge in the list) instead of a plain Income/Expense toggle, so platform gains/losses are distinguishable from cash moved in or out.
- **Investment Correction Transactions:** A "Correction" action on the Investments tab reconciles a platform's recorded balance to its real balance (entered by the user) by inserting an adjustment transaction tagged `Correction` — mirrors the existing bank Correction Transaction flow on the Transactions tab.
...
- **Desktop-width Layout:** The page body is capped at `max-w-7xl` and centered (matching Dashboard) instead of stretching edge-to-edge on wide screens. The Banks tab and the Investments tab's transaction list render as responsive multi-column grids (`md:`/`lg:`/`xl:` breakpoints) on desktop instead of staying a single narrow stacked list regardless of viewport width; the main Transactions table already had a dedicated desktop table view.
- **Single Category Breakdown:** The spending-by-category breakdown is shown once, as its own standalone "By Category" list card. It's no longer duplicated as a tab inside the grouped chart card (which now defaults to its "Over time" tab and still offers "By account").
...
- Platform-Specific UI/UX:
  ...
  - **iOS Native View (`Notes.ios.tsx`)**: Apple Notes aesthetic, grouped inset card lists, iOS header, swipe gestures, haptic feedback, 3D long-press touch context menus, screen navigation, elevated FAB new note button and AI organizer toolbar positioned above the LiquidTabBar.
...
- **3D Long-Press Ayah Action Menu with Slide-to-Select, Animated Tafseer Drawer & Habit Sync:** Long-pressing any ayah triggers an iOS-style 3D lift effect and opens a frosted glass context menu with 5 core actions (Hide Ayah session-only, Memorization Checkpoint, Reading Checkpoint, Bookmark to Notes, Tafseer). Supports native iOS hold-and-slide-to-select touch gestures with global touchmove/touchend tracking, haptic tick feedback, and direct release-to-choose execution. The Quran Fehres (Surah, Juz, and Page picker) is keyboard-aware with dynamic visual viewport adaptation and non-intrusive focus management on mobile iOS. Setting a Memorization (`mem_checkpoint`) or Reading (`read_checkpoint`) checkpoint immediately captures the exact Medina Mushaf page and Ayah number, synchronizes to `quran_khatmah_plans` and localStorage, updates the linked habit's description (e.g. `آخر موضع حفظ: سورة الجن (الآية 28) • صفحة 572`), and marks the habit as completed for today. Conversely, completing a Quran habit in the habit tracker advances the wird, updates the habit description with the next target page and Ayah, and updates the checkpoint accordingly.
...
- **12-Hour Non-Repeating Hadith of the Day (Mobile Only):** Displays an inspiring Hadith dialog on mobile every 12 hours from a non-repeating queue, featuring enhanced Arabic typography, Cairo font for the narrator, adaptive light and dark presentation, and zero desktop overlap (PC already features the Hadith widget in Dashboard).
- **Surah Title Banners:** Displays Quran chapter headers with authentic Islamic paper gradients and amber typography adapted dynamically for both light and dark themes.
...
- **Khatma Stats (إحصائيات الختمة):** Tracks total time spent in the mushaf reader (accumulated while the reader is open and the tab is visible, persisted to `quran_khatmah_plans.total_reading_seconds`), and separately counts completed full-Quran read-throughs (`reading_khatmas_completed`, incremented when the reading wird advances past page 604) and completed memorization passes (`memorization_khatmas_completed`, incremented when the memorization plan reaches its end boundary). Displayed as a stats card at the top of the Khatmah tab. Local-first with cross-device sync via `useQuranCloudSync` taking the max of local/remote per field so a fresh device or stale fetch can't regress progress.
- **Reading Position Preservation on Habit Completion:** Completing the daily reading-wird habit no longer blindly resets the tracked reading marker to ayah 1 of the newly-advanced page — if the user had already manually marked a further-along position while actually reading (via the "Reading Checkpoint" ayah action), that real position is preserved instead of being overwritten, so wird reminder notifications keep reflecting genuine progress rather than resetting on every habit checkbox tap.
- **Fullscreen Mushaf Page-Turn Scroll Fix:** In the immersive fullscreen reader, turning from the end of one page to the next now resets the fullscreen container's own scroll position (it scrolls independently of the window) and lands the new page's first ayah at the top of the viewport instead of centering it, so auto-scroll page transitions land cleanly at the top of the new page rather than mid-scroll.
...
- **Auto-Completed Habit Sync with Night Grace Window:** Finishing a Morning/Evening/Sleep Azkar category auto-marks the matching user Habit as done for the day. Evening and Sleep Azkar completed between midnight and 6 AM are credited to the previous calendar day's habit instead of the new day's, so reading night azkar late doesn't leave the new day's habit falsely pre-completed before anything has been done on it.
...
- **Smart Awake-Time Task Distribution Engine:** Analyzes action items against user wake/bedtimes (from sleep tracking metrics), existing scheduled tasks, and calendar events to distribute tasks into open, conflict-free awake slots with recommended lists (`Work`, `Learn`, etc.) and tags (`#servixa`, etc.). Weekly redistribution and the Tasks page's "Reorganize This Week" mode both harvest overdue tasks as well as current-week items, regardless of whether the task was created manually or by AI/Brain Dump.
...
- **Remote Brain Dump Append Endpoint (`append-braindump`):** Supabase Edge Function and PostgreSQL RPC (`append_to_daily_braindump`) allowing iOS Apple Shortcuts, Siri, OpenClaw bots (`BD <text>`), and external webhooks to append thoughts directly to today's unified Brain Dump note with automated timestamps.
- **Raw Thought Carried Into Task Details:** Any task created from a Brain Dump — whether via the manual "Add Task" action, the AI Organizer, a batch "Sync Tasks" export, or the "Organize & File" / nightly auto-organizer flow — has its `description` populated with the original raw thought text it was extracted from, so the full context is always one tap away on the task itself instead of being lost once the note is organized.
...
- **Batch Processing & IndexedDB Persistence:** Batch task deletion via `useBatchDeleteTasks` purges target tasks and cascades child subtasks across React Query cache, Supabase DB, and offline IndexedDB persistence (`idbSaveTasks`) simultaneously, preventing deleted tasks from resurfacing upon page reload.
- **Tag-Aware Brain Dump Selection:** The "Select Brain Dump" action intelligently detects if the user is already inside the `#braindump` tag view and selects all visible tasks across active, completed, and won't-do sections, while also detecting notes originating via `source_note_id`.
...
- **Timestamps:** Every chat message shows its send time, and the Chat History sidebar shows each saved thread's last-activity time (or date + time for older threads), so past conversations are easy to place in time.
- **Resilient Error Display:** If an AI provider or an intermediary gateway fails with a non-JSON (e.g. HTML) error body, the raw markup is never rendered into the chat — it would otherwise inject foreign styling into the page — and a plain, readable error message is shown instead.
- **Brain Dump Auto-Organizer Opt-In:** The nightly server-side job that summarizes Brain Dump notes with AI is off by default and only runs for users who explicitly enable "Auto-organize Brain Dump notes" under Settings > AI Assistant > Automations. It only ever processes that user's own notes using that user's own AI keys.
