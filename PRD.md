...
- **3D Long-Press Ayah Action Menu & Tafseer Drawer:** Long-pressing any ayah triggers an iOS-style 3D lift effect and opens a frosted glass context menu with 5 core actions: Hide Ayah (session-only self-testing), Set Memorization Checkpoint, Set Reading Checkpoint, Bookmark to Notes, and Tafseer (instant Tafsir Al-Muyassar sheet drawer).
- **Full Offline-First Quran & App Startup:** All 604 Mushaf pages are cached persistently in IndexedDB (`quran_pages` store) with throttled background prefilling. Offline queueing enables progress logging and notes creation with zero internet connection, and instant localStorage session bootstrapping guarantees app startup in <2s offline.
