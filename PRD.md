...
- **3D Long-Press Ayah Action Menu with Slide-to-Select & Animated Tafseer Drawer:** Long-pressing any ayah triggers an iOS-style 3D lift effect and opens a frosted glass context menu with 5 core actions (Hide Ayah session-only, Memorization Checkpoint, Reading Checkpoint, Bookmark to Notes, Tafseer). Supports native iOS hold-and-slide-to-select touch gestures. Selected ayah uses a clean, non-disruptive underline without layout shift or wiggles, and Tafsir Al-Muyassar opens in a smooth slide-in animated bottom sheet.
